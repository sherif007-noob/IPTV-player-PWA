...
function getDurationFromXtreamUrl(upstreamUrl: string): Promise<number | null> {
  ...
  const promise = (async () => {
    try {
      ...
      return null;
    } finally { durationPromises.delete(upstreamUrl); }
  })();
...
function installMediaPropertyBridges() {
  ...
  Object.defineProperty(HTMLVideoElement.prototype, 'duration', {
    configurable: true, enumerable: durationDescriptor.enumerable,
  ...
  Object.defineProperty(HTMLVideoElement.prototype, 'currentTime', {
    configurable: true, enumerable: currentTimeDescriptor.enumerable,
    get(this: HTMLVideoElement) {
...
function installUserSeekDetection() {
  document.addEventListener('pointerdown', (event) => {
    ...
    if (target?.closest('input[type="range"]')) userSeekUntil = Date.now() + 1200;
  }, true);
  document.addEventListener('keydown', (event) => {
    if (event.key === 'ArrowLeft' || event.key === 'ArrowRight' || event.key === 'MediaRewind' || event.key === 'MediaFastForward') userSeekUntil = Date.now() + 1200;
  }, true);
}
  ...
  document.addEventListener('seeking', (event) => {
    ...
    try {
      ...
      const playAfterLoad = () => {
        ...
        if (!wasPaused) { const p = video.play(); if (p) p.catch(() => {}); }
      };
      video.addEventListener('loadedmetadata', playAfterLoad, { once: true });
    } finally { window.setTimeout(() => seekInProgress.delete(video), 1500); }
  }, true);
}
